...
public class ReloadPurseTerminal {
    ...
    public void run() {
        try {
             ...
             * Enables exchanging APDUs with a smart card.
             */
    ...
    public void testReload(CardChannel applet) {
        
        System.out.println("Starting Reload protocol");
        
        try {
            CommandAPDU capdu = new CommandAPDU(0x00, Constants.START_RELOAD_PROTOCOL, 0x00, 0x00, null, 0);
            // Transmit the specified CommandAPDU with the instruction byte for the reload protocol and return the response APDU
            ResponseAPDU rapdu = applet.transmit(capdu);
            System.out.println(rapdu.toString());
            
            // set PIN code if first reload 
            // TODO
            // else
            // send reload amount
            ...
            if (sig.verify(rapdu.getData())) {
                System.out.println("Received a valid reload signature");
            } else {
                System.err.println("Received invalid reload signature");
            }
